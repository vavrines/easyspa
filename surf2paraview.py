...
class UGridSource(VTKPythonAlgorithmBase):
    ...
    def RequestInformation(self, request, inInfo, outInfo):
        ...
        if tsteps:
            info.Set(
                vtk.vtkStreamingDemandDrivenPipeline.TIME_STEPS(), tsteps, len(tsteps)
            )
            info.Set(
...
def read_points(sif, num_points, num_elements, ug, three_d_file):
    ...
    for line in sif:
        ...
        if s and len(s.split()) == num_items_per_line:
            ...
            if s.split()[0].lower() == "triangles" or s.split()[0].lower() == "lines":
...
def read_elements(sif, num_points, num_elements, ug, three_d_file):
    ...
    if ug.GetNumberOfCells() != num_elements:
        print("Error reading SPARTA surf input file")
        print(
            "The number of elements in Triangles or Lines section: ",
            ug.GetNumberOfCells(),
        )
    ...
    for line in sif:
        ...
        if s and len(s.split()) == 2:
            ...
            if len(s.split()) != 1:
                ...
                read_points(sif, num_points, num_elements, ug, three_d_file)
            elif s.split()[0].lower() == "triangles" or s.split()[0].lower() == "lines":
                read_elements(sif, num_points, num_elements, ug, three_d_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "sparta_surf_file", help="SPARTA surface geometry input file name"
    )
    parser.add_argument("paraview_output_file", help="ParaView output file name")
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "-r", "--result", help="Optional list of SPARTA dump result files", nargs="+"
    )
    group.add_argument(
        "-f",
        "--resultfile",
    ...
    if args.result:
        ...
        try:
            ...
            print(
                "Unable to open SPARTA result file input list file: ", args.result_file
            )
            sys.exit(1)
    ...
    if args.exodus:
        ...
        for time in sorted(time_steps_dict.keys()):
            ...
            filepath = os.path.join(
                ...
                args.paraview_output_file + "_" + str(time) + ".vtu",
            )
