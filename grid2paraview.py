        ...
        for line in grid_file_handle:
            ...
            if self.number_of_levels > 0:
                for level in range(1, self.number_of_levels + 1):
                    level_string = self._get_grid_file_level_definition_string() + str(
                        level
                    )
...
def create_and_write_grid_chunk(
    chunk_id, chunk_info, num_chunks, grid_desc, time_steps_dict, output_file
):
    ug = process_grid_chunk(
        chunk_id, chunk_info, num_chunks, grid_desc, time_steps_dict, output_file
    )
    write_grid_chunk(ug, chunk_id, num_chunks, grid_desc, time_steps_dict, output_file)
...
def process_grid_chunk(
    ...
    if grid_desc["dimension"] == 3:
        ug = create_3d_amr_grids(
            grid_desc, 1, 0, 0, origin, spacing, ndims, chunk_info, ""
        )
    elif grid_desc["dimension"] == 2:
        ug = create_2d_amr_grids(
            grid_desc, 1, 0, 0, origin, spacing, ndims, chunk_info, ""
        )
...
def write_grid_chunk(ug, chunk_id, num_chunks, grid_desc, time_steps_dict, output_file):
    if not ug:
        return
    ...
    for idx, time in enumerate(sorted(time_steps_dict.keys())):
        ...
        filepath = os.path.join(
            output_file, output_file + "_" + str(chunk_id) + "_" + str(time) + ".vtu"
        )
        writer.SetFileName(filepath)
        writer.Write()
        pt.report_collective_time(
            "grid file write time %s (wall clock time) for step " + str(idx)
        )
...
def create_3d_amr_grids(
    ...
    if level_contains_refined_cells(level, grid_desc, dashed_id):
            ...
            for j in range(ndims[1] - 1):
                ...
                for i in range(ndims[0] - 1):
                    ...
                    if refine:
                        ...
                        r_ug = create_3d_amr_grids(
                            grid_desc,
                            level + 1,
                            bit_mask + parent_bit_mask,
                            refined_cell_index,
                            [xl, yl, zl],
                            r_spacing,
                            r_ndims,
        ...
        return build_3d_grid(
            parent_bit_mask, parent_id, origin, spacing, ndims, chunk_info, grid_desc
        )
...
def cell_is_inside_chunk(cell_first_level_loc, chunk_info):
    ...
    return (
        xloc >= chunk_info["x"][0]
        and xloc <= chunk_info["x"][1]
        and yloc >= chunk_info["y"][0]
        and yloc <= chunk_info["y"][1]
        and zloc >= chunk_info["z"][0]
        and zloc <= chunk_info["z"][1]
    )
...
def is_3d_cell_refined(
    level, i, j, k, cell_index, grid_desc, dashed_id, xi, yi, zi, r_spacing, r_ndims
):
...
def is_2d_cell_refined(
    level, i, j, cell_index, grid_desc, dashed_id, xi, yi, r_spacing, r_ndims
):
    if "parent_grid" not in grid_desc:
        Px = grid_desc["create_grid"][level + 1]["Px"]
        Py = grid_desc["create_grid"][level + 1]["Py"]
                ...
                if d <= math.fabs(rhs):
                    ...
                    for pj in range(2):
                        ...
                        for pi in range(2):
                            points.InsertNextPoint(
                                p1 - a1 + pi * spacing[0], p2 - a2 + j_offset, 0.0
                            )
                    ...
                    if parent_id == 0:
                        ...
                        if parent_bit_mask == -1:
                            ...
                            gids.InsertNextTuple1(
                                index * (2**parent_bit_mask) + parent_id
                            )
...
def cells_on_slice_planes(
    ...
    if grid_desc["dimension"] == 3:
        return find_3d_intersected_cells(
            intersecting_planes,
            parent_bit_mask,
            parent_id,
            origin,
        ...
        return find_2d_intersected_cells(
            intersecting_planes,
            parent_bit_mask,
            parent_id,
            origin,
...
def build_2d_grid(
    ...
    for j in range(ndims[1]):
        ...
        for i in range(ndims[0]):
            points.InsertNextPoint(
                origin[0] + i * spacing[0], origin[1] + j_offset, 0.0
            )
    ug.SetPoints(points)
...
def read_grid_levels(gl_array, grid_desc, level):
    if len(gl_array) % 8 or gl_array[0].lower() != "level" or int(gl_array[1]) != level:
...
def read_grid_description_file(sif, grid_desc):
    for line in sif:
        ...
        if s.lower()[:9] == "dimension" and len(s.split()) == 2:
            ...
            norm = math.sqrt(
                math.pow(p["nx"], 2) + math.pow(p["ny"], 2) + math.pow(p["nz"], 2)
            )
            p["nx"] = p["nx"] / norm
            p["ny"] = p["ny"] / norm
            p["nz"] = p["nz"] / norm
            ...
            if (
                float(s.split()[1]) < float(s.split()[2])
                and float(s.split()[3]) < float(s.split()[4])
                and float(s.split()[5]) < float(s.split()[6])
            ):
...
class ParallelTimer:
    ...
    def report_collective_time(self, message):
        ...
        if self.rank == 0:
            print("")
            print(
                "Average " + message % timedelta(seconds=total_time / float(self.size))
            )
...
def report_collective_grid_sizes(ug):
    ...
    if MPI.Is_initialized():
        ...
        if rank == 0:
            print(
                "Average grid cell count over MPI ranks: {:.1e}".format(
                    total_cell_count / float(size)
                )
            )
            print(
                "Minimum grid cell count over MPI ranks: {:.1e}".format(min_cell_count)
            )
            print(
                "Maximum grid cell count over MPI ranks: {:.1e}".format(max_cell_count)
            )
            print(
                "Average grid memory used over MPI ranks: {} MB".format(
                    (total_mem_used / float(size)) / 1000.0
                )
            )
            print(
                "Minimum grid memory used over MPI ranks: {} MB".format(
                    min_mem_used / 1000.0
                )
            )
            print(
                "Maximum grid memory used over MPI ranks: {} MB".format(
                    max_mem_used / 1000.0
                )
            )
...
def run_pvbatch_output(params_dict):
    ...
    for idx, chunk in enumerate(chunking[start : stop + 1]):
        g = process_grid_chunk(
            idx, chunk, len(chunking), grid_desc, time_steps_dict, paraview_output_file
        )
        if g:
    ...
    if catalystscript is not None:
        if rank == 0:
            print(
                "Calling Catalyst over "
                + str(len(time_steps_dict))
                + " time step(s) ..."
            )
        import coprocessor
        ...
        for idx, time in enumerate(sorted(time_steps_dict.keys())):
            ...
            pt.report_collective_time(
                "catalyst output time %s (wall clock time) for step " + str(idx)
            )
        coprocessor.finalize()
    else:
        if rank == 0:
            print(
                "Writing grid files over "
                + str(len(time_steps_dict))
                + " time step(s) ..."
            )
        write_grid_chunk(
            ug, rank, size, grid_desc, time_steps_dict, paraview_output_file
        )
...
if __name__ == "__main__":
    ...
    if local_proc_id == 0:
        ...
        parser.add_argument(
            "-c",
            "--catalystscript",
            help="Run a ParaView Catalyst Python script with pvbatch",
        )
        ...
        try:
            ...
            print(
                "Unable to open SPARTA surf input file: ",
        ...
        if args.catalystscript:
            if num_procs == 1:
                print(
                    "Error: ParaView Catalyst only available with pvbatch and more than one MPI rank"
                )
                sys.exit(1)
            try:
                ...
                print(
                    "Error: Unable to open Catalyst Python file: ", args.catalystscript
                )
                sys.exit(1)
        ...
        if "read_grid" not in grid_desc and "create_grid" not in grid_desc:
            print(
                "Error: grid description file does not have a read_grid or a create_grid statement: ",
                args.sparta_grid_description_file,
            )
            sys.exit(1)
        ...
        if "slice" not in grid_desc:
            if (
                os.path.isfile(args.paraview_output_file + ".pvd")
                and not args.catalystscript
            ):
                print(
                    "ParaView output file exists: ", args.paraview_output_file + ".pvd"
                )
                sys.exit(1)
        else:
            for idx, slice in enumerate(grid_desc["slice"]):
                file_name = (
                    ...
                    + str(idx)
                    + "-"
                    + str(round(slice["nx"], 4))
                    + "_"
                    + str(round(slice["ny"], 4))
                    + "_"
        ...
        if args.result:
            ...
            try:
                ...
                print(
                    "Unable to open SPARTA result file input list file: ",
                    args.result_file,
                )
                sys.exit(1)
    ...
    if local_proc_id == 0:
        if args.catalystscript is None:
            if "slice" not in grid_desc:
                if num_procs == 1:
                    write_pvd_file(
                        time_steps_dict, args.paraview_output_file, len(chunking)
                    )
                else:
                    write_pvd_file(
                        time_steps_dict, args.paraview_output_file, num_procs
                    )
            else:
